...
export default function DaftarMitra() {
  ...
  const handleDelete = async (email: string) => {
    try {
      const res = await fetch(`/api/staf/mitra/${encodeURIComponent(email)}`, {
        method: 'DELETE'
      });
      const data = await res.json();
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-[var(--color-border-light)]">
                {loading ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center text-xs text-[var(--color-text-muted)]">Memuat data...</td>
                  </tr>
                ) : mitra.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center text-xs text-[var(--color-text-muted)]">Belum ada mitra terdaftar.</td>
                  </tr>
                ) : mitra.map((item) => (
                  <tr key={item.email_mitra} className="hover:bg-[var(--color-bg-subtle)]/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-xs text-[var(--color-text-muted)]">
                        {new Date(item.tanggal_kerja_sama).toLocaleDateString()}
                      </p>
                    </td>
