  ...
  return (
    <div className="max-w-4xl mx-auto p-6 md:p-10 font-sans">
      ...
      <div className="flex items-center gap-2 text-xs text-[var(--color-text-muted)] mb-6">
        <Link href="/staf/klaim" className="hover:text-[var(--color-primary)] transition-colors">Kelola Klaim</Link>
        <span>/</span>
        <span>Detail Klaim #{id}</span>
      </div>

      <div className="mb-6 flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-title)] tracking-tight mb-1">Detail Klaim #{id}</h1>
          <p className="text-sm text-[var(--color-text-muted)]">Informasi lengkap pengajuan klaim missing miles</p>
        </div>
        <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-semibold ${STATUS_STYLE[currentStatus]}`}>
          {currentStatus}
        </span>
      </div>
